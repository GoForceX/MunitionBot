...
async def handle_message(bot: Bot, event: Event, state: T_State):
    ...
    if args[0] == "bf1" or args[0] == "bf4" or args[0] == "bfv":
        if args[0] and args[1]:
            logger.debug(
                "Requesting: https://api.gametools.network/" + args[0] + "/servers/?name=" + args[1] + "&lang=" + (
                    "zh-cn" if args[0] == 'bfv' else "zh-tw") + "&region=all&platform=pc&limit=8")
            async with httpx.AsyncClient() as client:
                try:
                    await query.send("稍等下，正在获取您请求的信息……")
                    resp = await client.get(
                        "https://api.gametools.network/" + args[0] + "/servers/?name=" + args[1] + "&lang=" + (
                            "zh-cn" if args[0] == 'bfv' else "zh-tw") + "&region=all&platform=pc&limit=8", timeout=10.0)
                ...
                for i in range(4):
                    for j in range(2):
                        ...
                        drawer.text((83 + 416 * j, 40 + 163 * i), "服务器名：" +
                                    str(result[2 * i + j]["prefix"][:22] + (
                                        '...' if len(result[2 * i + j]["prefix"]) > 22 else '')) +
                                    "\n状态：" +
