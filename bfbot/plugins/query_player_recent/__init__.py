...
async def handle_message(bot: Bot, event: Event, state: T_State):
    ...
    if args[0] == "bfv":
        if args[0] and args[1]:
            logger.debug(
                "Requesting: https://api.tracker.gg/api/v1/" + args[0] + "gamereports/origin/latest/" + args[1])
            async with httpx.AsyncClient() as client:
                try:
                    await query.send("稍等下，正在获取您请求的信息……")
                    resp = await client.get(
                        "https://api.tracker.gg/api/v1/" + args[0] + "/gamereports/origin/latest/" + args[1],
                        timeout=10.0)
                ...
                for i in range(4):
                    for j in range(2):
                        ...
                        drawer.text((83 + 416 * j, 40 + 163 * i), "服务器名：" +
                                    str(result[2 * i + j]["serverName"][:22] + (
                                        '...' if len(result[2 * i + j]["serverName"]) > 22 else '')) +
                                    ("\n模式：" +
