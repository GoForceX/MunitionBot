...
async def handle_message(bot: Bot, event: Event, state: T_State):
    ...
    if args[0] == "bf1" or args[0] == "bf4" or args[0] == "bfv":
        if args[0] and args[1]:
            logger.debug("Requesting: https://api.gametools.network/" + args[0] + "/stats/?name=" + args[1] + "&lang=" + ("zh-cn" if args[0] == 'bfv' else "zh-tw"))
            async with httpx.AsyncClient() as client:
                try:
                    await query.send("稍等下，正在获取您请求的信息……")
                    resp = await client.get("https://api.gametools.network/" + args[0] + "/stats/?name=" + args[1] + "&lang=" + ("zh-cn" if args[0] == 'bfv' else "zh-tw"), timeout=8.0)
